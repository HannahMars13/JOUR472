...
A1: I found 76 records.

```{r}
pg_schools <- payments %>%
  filter(str_detect(vendor_name, "PG ") | str_detect(vendor_name, "PRINCE GEORGES"))

pg_schools <- pg_schools %>%
  filter(vendor_name == "BD OF EDUCATION OF PG CO" | vendor_name == "BOARD OF EDUCATION OF PRINCE GEORGES COUNTY MD" | vendor_name == "PG COUNTY PUBLIC SCHOOL" | vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOL" | vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOLS" | vendor_name == "PRINCE GEORGES COUNTY SCHOOL" | vendor_name == "PRINCE GEORGES PUBLIC SCHOOLS")
```
...
A2: 2022 had the largest total amount, at \$1,578,857,325. The years 2012 and 2016 are missing from the data, which makes me wonder if they were just not included in the data for some reason of if something happened that caused there to be no data for those years.

```{r}
pg_payments <- pg_schools %>%
...
A3. It appears as though there are many entries where it's the same vendor just spelled in a slightly different way, so I would need to choose one standardization for the name of a vendor and change the rest of the entries to reflect that. Some examples of vendors that should be combined into one are "LIPMAN FRIZZELL & MITCHELL-LLC" and "LIPMAN FRIZZELL & MITCHELL LLC"; "HORIZON ENGINEERING ASSOCIATES LLP" and "HORIZON ENGINEERING ASSOCIATES"; and "GRIMM + PARKER ARCHITECTS" and "GRIM & PARKER ARCHITECTS". For the first example, I would use a function that would allow me to remove the dash in the one variation. For the second one, I would use a substring function to remove the LLP at the end of the one variation. For the third example, I would use a str_replace to fix both the spelling error and the different punctuation use in the second example.
...
A4: The highest amount was in 2009, at \$14,643,721. There was a pretty substantial decrease to \$7,870,123 in 2010, and it remained around that number in all the following years. Most recently, in 2022, the amount was \$7,069,212.

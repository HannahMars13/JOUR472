...
**A1. Link to chart: <https://datawrapper.dwcdn.net/48Y4r/1/>**

I chose to do a grouped bar graph that showed the percentages for each
of the candidates by county, for I felt that having the values for the
same county right next to each other did the best job of showing the
difference in percent.

```{r}
county_votes <- read_csv("data/md_gov_county.csv") |> clean_names()
county_votes_18 <- read_csv("data/md_gov_county_18.csv") |> clean_names()

all_county_votes <- county_votes_18 |>
  left_join(county_votes, by = c("geoid", "county")) |>
  mutate(
    pct_cox = (cox/(cox+moore+lashar+wallace+harding+sefcik+write_ins.y))*100,
    pct_hogan = (hogan/(hogan+jealous+quinn+schlakman+write_ins.x))*100
  )
...
**A2. Link to graph: <https://datawrapper.dwcdn.net/OK7KI/1/>**

I think the most noticeable aspect of this chart is the fact that mail
voting used to be less than 10% of votes, but following the pandemic, it
seems to have become a more popular option.

```{r}
type_of_votes <- read_csv("data/md_votes_by_type.csv") |> clean_names()

pct_type_of_votes <- type_of_votes |>
  mutate(pct_early = (early/total)*100,
         pct_election_day = (election_day/total)*100,
         pct_mail = (mail/total)*100) |>
  select(-early, -election_day, -mail, -total)

write_csv(pct_type_of_votes, "data/pct_type_of_votes.csv")
```
...
**A3. Link to map: <https://datawrapper.dwcdn.net/OtcnE/1/>**

It seems that the zip codes near the major cities of Montgomery County
are the ones with the highest percentage of overdose calls. I would want
to know more about the demographics of this area to see if there is any
correlation.

```{r}
moco_overdoses <- read_csv("data/montgomery_2022_overdoses.csv")

overdoses_by_zip <- moco_overdoses |>
  group_by(zip) |>
  summarize(total = n()) |>
  mutate(pct_calls = (total/659)*100)

write_csv(overdoses_by_zip, "data/moco_overdoses_by_zip.csv")
```

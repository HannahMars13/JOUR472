...
```{r}
bc_911_overdose_calls <- read_csv("data/tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) %>%
  clean_names() %>%
...
**A1. July 14 and October 4 were tied for the most calls in 2022 at 23. Considering there are 366 rows in this data frame and the data is meant to span a full year, it does not seem as though there are any days with no overdose calls.**

```{r}
calls_per_date <- bc_911_overdose_calls %>%
  group_by(date) %>%
...
**A2. It appears that the number of calls is generally higher on the weekend, with Sunday having the highest number of calls. The numbers drop off a bit in the middle of the week, although the day with the lowest number of calls is actually Friday.**

```{r}
bc_911_overdose_calls <- bc_911_overdose_calls %>%
  mutate(day_of_week = wday(date))

calls_per_weekday <- bc_911_overdose_calls %>%
...
**A3. The locations with the most calls are a random street address and the addresses of two different Baltimore County Police Precincts. I don't feel as confident in the counts by location since some locations have the name of a place/company followed by the street address, some have the precinct number if it's a police department, and others just have a street address. Also, it doesn't include the city, which could possibly create confusion if there are duplicate street names.**

```{r}
calls_per_location <- bc_911_overdose_calls %>%
  group_by(location) %>%
  summarize(count_calls = n()) %>%
  arrange(desc(count_calls))
```
...
**A4. It seems like there are a lot of overdose calls that have occurred at various police precincts in Baltimore County, so I would be interested to explore what exactly is going on there and how the numbers at police departments compare to those at other types of locations.**

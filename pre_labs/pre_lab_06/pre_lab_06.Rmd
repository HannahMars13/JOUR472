...
**Answer:** While they're not all in the same order, they do have the same column structure. The only noticeable difference is that the "Total" column for 2022 is not in all caps, unlike the other three files.
...
**Answer:** The "unaffiliated" and "other" categories saw a pretty substantial jump within this time frame. Also, while the number of registered Republicans rose more steadily, the number of registered Democrats jumped up by several thousand between 2018 and 2020 and then again between 2020 and 2022.
...
**Answer:** There are still 24 rows, which is the number of rows that there should be considering the two dataframes being combined both have 24 rows that are organized by county, which is what they are being joined on.
...
**Answer:** It shows the number of people over 18 or older as well as a breakdown of racial information, with each row representing a different county.
...
**Answer:** The county with the highest 18+ Black population is Prince George's County, which is 77% Democrats.
...
maryland_population_with_voters_and_demographics |>
  ...
  select(COUNTY, POP2020, pct_black_18_plus, pct_dems)
```
...
**Answer:** The percentages for all counties are as a whole much smaller, with the highest only reaching 19% in Howard County. Montgomery County is the second highest percent at 16%, but what is strange is that the third highest percent is only 6%. There is clearly not as strong of a correlation between Asian population and percentage of Democrats as there was with the Black population and Democrats.
...
**Answer:** The data type of this column is a double, which is a numeric type.
...
**Answer:** ZCTA5N is now a character type, so it will now be interpreted as text.

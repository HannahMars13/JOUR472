...
**Answer: The first line is selecting the umd_courses dataset. The second line is taking all the different values of terms and grouping the results by these values. The third and fourth lines are adding up all the records in each term in the dataset.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: This time, there is an extra line to arrange the column count_classes in descending order. This means that now the results are sorted from greatest to least based on the value of count_classes.**
...
**Answer: The output is different since now it has both the term and the department instead of just the term. The second line of code changed it by grouping by both the term and department as opposed to just the term, as it was in the earlier example. This led to a lower number for each result in count_classes since now it is counting every time there was an instance of that specific combination of term and department. Also, the last line appears to have put the results in alphabetic order.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: This time, a new column known as total_seats was created by using the sum operation on seats within each term. The results were then put into descending order by total_seats in the last line of code.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: New operations were done on "seats" to determine the mean and median, which provided the viewer with more information regarding the average number of seats available in a class. The top terms have almost the same mean number of seats, which makes sense because their values for count_classes and total_seats are fairly close.**
...
**Answer: For this codeblock, the results were sorted by department instead of by term, and the minimum and maximum numbers of seats were added to provide more perspective on how many people can be in a class for each department. This information is more specific than the previous examples since it has the count of classes per department as opposed to the total number of classes available in a given term. The data is sorted by total seats from greatest to least.**
...
1.  Which departments have the largest number of classes?

2.  How do some classes have a maximum number of seats that is in the thousands?

3.  How many total departments are there?

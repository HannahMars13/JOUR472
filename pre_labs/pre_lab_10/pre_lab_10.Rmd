...
**Answer: This data has eight columns: one integer type, four character type, two double type, and one multipolygon type.**
...
**Answer: There is an object ID, two types of zip code entries, the name of the zip code, a character column called "existing", and three columns related to spatial data: shape area, shape length, and geometry.**
...
**Answer: It shows a map of Maryland with the outlines for all of its zip codes, and there are latitude and longitude lines.**
...
**Answer: It gives a warning about how to enable caching of data.**
...
**Answer: There are a lot of columns that seem to contain spatial information about each county in the US.**
...
**Answer: This new dataframe only has the 24 counties in Maryland, which were selected based on their FP number.**
...
**Answer: This produced a map similar to the previous one of Maryland, but instead of having the outlines of each zip code, it has the outlines for each county.**
...
**Answer: This map now has both the county and zip code outlines overlapped.**
...
**Answer:** **There are two columns in this dataframe: the zip code, and a corresponding number of notices of intent to foreclosure from March 2023.**
...
**Answer: This new dataframe has the foreclosure data combined with the data on Maryland zip codes.**
...
**Answer: This map of Maryland has a different shade of blue in the gradient for each zip code based on the number of notices of intent to foreclosure.**
...
**Answer: Here we are loading more of the necessary libraries for creating maps and getting census data.**
...
**Answer: Using census data, we now have dataframes with information on how many foreclosure notices there are per county and the population for each county.**
...
**Answer: The Maryland county population dataframe now has an extra column with the geometry data for each county.**
...
**Answer: Both dataframes displayed below show information about Maryland counties but are formatted differently.**
...
**Answer: The names have been fixed so that they are more consistent with the other dataframe, and only the ncessary columns have been selected.**
...
**Answer: The population and county dataframes have now been joined into one six-column dataframe with all of the information for each county.**
...
**Answer: The combined dataframe now has a column with the rate of foreclosure notices per county, which was calculated using the notices and population columns. It can be seen that Charles County has the highest rate of foreclosure notices.**
...
**Answer: Below is a map of Maryland with each county labeled. The gradient is based on the rate of foreclosure notices.**
...
**Answer: This map is different than the above one since the different values have a completely different color as opposed to being on a gradient. Also, the counties are no longer labeled. While this does make it easier to see the differences in values, it is perhaps less exact.**
...
**Answer: It seems that the northernmost counties generally have lower rates, as do the ones at the farthest tips of the state. The middle to more southern part of the state has the highest rates.**

...
**Answer: That's important so we don't overwrite the original dataframe and instead create a separate object, allowing us to continue using the original dataframe for other analysis.**
...
**Answer: R now identifies the data in this column as being of the \<date\> type, and it changed the formatting to year-month-date, replacing the slashes with dashes.**
...
**Answer: The use of the floor_date function changed all of the date entries so that the day would be 01, thus allowing the group_by function to group all of the entries by month.**
...
**Answer: This code block used the mutate function to create two new columns, called total_votes and pct_moore. The values for the total_votes column were gotten by adding together the values of the five columns of votes. The values for pct_moore were gotten by taking just the column of Wes Moore's number of votes and dividing that value by the value in the total_votes column.**
...
**Answer: In the previous task, the records were in alphabetical order by county. In this task, the arrange() function was used to put the records in descending order based on the pct_moore column, so now the counties with the highest percentages of votes for Wes Moore are at the top of the list.**
...
**Answer: This answer could be more useful depending on what you are looking for, for looking into a county of which only 22% of people voted for the winning candidate could lead to a pretty interesting story.**
...
**Answer: Two tidyverse functions similar to str_to_upper are str_to_lower and str_to_title. str_to_lower converts the character object to all lowercase, and str_to_title makes it so that only the first letter of each word in the object is capitalized.**
...
**Answer: The mutate created a column called state, and it uses a case_when statement to determine what the output will be in each row of the state column. The str_detect function first looks in the address column to find "Maryland." If it finds it, the state column for that row will output "MD." If it does not find it, the code will move onto the next line and so on until the str_detect catches the state name that appears in that row's address. If str_detect does not find any of those strings in the address column, it will fill in "NA" and then move onto the next row.**
...
**Answer: It does make sense the Maryland addresses would have the highest total expenses, but I was not expecting states like Washington and California to be so close to the top of the list.**
...
1.  In the maryland_expenses dataframe, what other states/districts can be found within the rows that came up as NA in the state column?

2.  In the umd_courses dataframe, how many different "Independent Study" classes were offered in the most recent term?

3.  In the general_22 dataframe, which candidate won in each county?

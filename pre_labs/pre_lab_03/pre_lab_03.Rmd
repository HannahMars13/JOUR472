...
**Answer: The first line mentions that there are parsing issues. When I called "problems()" on the payments data frame, I saw that there are potential problems with the data types in the data frame.**
...
**Answer: No, this time there are no parsing errors.**
...
**Answer: The "Vendor Name" field is a character, the "Amount" field is a double, and the "Date" column is a character.**
...
**Answer: There are 549 zip codes that don't have five characters. A possible explanation is that the zip codes started with a 0, so the program automatically removed it, or the numbers were cut off by something else.**
...
**Answer: There are some entries where the Category column entry is NA.**
...
**Answer: There is a very wide range in values, with only two values in the Amount column that are in the double digits while the highest values go all the way to the 24,000 range.**
...
**Answer: It made all of the headers lowercase and added an underscore each time there was originally a space.**
...
**Answer: There are 50 records that each have one possible duplicate, for all 50 have a dupe_count of 2, meaning there are two records with that same data.**
...
**Answer: The new dataframe has 17,740 rows as opposed to the original 17,765.**
...
**Answer: Now all except a few outliers have five digits, and the ones that are supposed to start with a 0 have it included instead of it getting removed.**
...
**Answer: The zip code that has gotten the most money from the Maryland Tourism Board is 21202, which makes sense because that is Baltimore's zip code, and Baltimore is the most populated city in Maryland and a major location for tourism.**

```{r}
md_tourism_board <- cleaned_md_grants_loans |>

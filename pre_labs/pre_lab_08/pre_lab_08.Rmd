...
**Answer: There are 3822 rows and 4 columns.**
...
**Answer: In this code, we are creating a new dataframe in which the 911 calls are grouped by month. First we need to mutate a month column using the date column. Then we group the calls by month and get a count for each month. Then we arrange the count for each month in descending order.**
...
**Answer: It created a blank gray box since we haven't told it any details about what kind of plot we want yet.**
...
**Answer: There is now a basic bar chart with the months on the x axis and the count of calls on the y axis. There is a grid in the background but no other aesthetic features such as different colors.**
...
**Answer: The months have now been rearranged from month order to being in order of least to most calls. Also, the name of the x axis has changed from "month" to "reorder(month, total_calls)".**
...
**Answer: The axes got flipped, so the months are now on the y axis with the count on the x axis. The months are arranged top to bottom, most to least calls.**
...
**Answer: The background is a bit lighter, with the grid no longer being filled in, and the text is slightly fainter.**
...
**Answer: The background is now light blue, and there are white horizontal lines instead of a grid. The text is darker and bigger, and there are tick marks at each 100 calls.**
...
**Answer: The labels for the axes have been fixed to be clearer, and there is now a large bolded title for the graph as well as a caption in the bottom right corner.**
...
**Answer: The Baltimore 911 calls have been grouped by date, so there is a two-column table with date and total calls.**
...
**Answer: This displays a basic black and white line graph. The line graph currently looks very cramped and spiky, and since the x axis only specifies months, it is difficult to see any patterns within short periods of time.**
...
**Answer: All of the labels on the x axis are overlapping each other since now we have a label for each individual day, and the grid in the background has become very tight.**
...
**Answer: The date labels have been tilted, so it is now much easier to read what they say. It is still quite cramped, but it is at least readable.**
